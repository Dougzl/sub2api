...
> **現在のデフォルト配備方式:** Sub2API は現在、**組み込み SQLite3** をデフォルトで使用しており、単一ノード起動では **PostgreSQL / Redis が不要** です。そのため、リリース版バイナリを使って **Windows** 上で直接起動できます。
...
| データベース | SQLite3（組み込み・デフォルト） |
| キャッシュ/キュー | デフォルトは単一ノード内蔵モード、Redis は一部構成で任意 |
...
### クイックスタート: Windows / 単体バイナリ

ローカルまたは単一の Windows マシンで Sub2API を動かしたいだけなら、GitHub Releases の配布バイナリをそのまま使えます。

#### 手順

1. GitHub Releases から Windows 用リリースをダウンロード
2. 任意のディレクトリに展開（例: `D:\sub2api`）
3. `sub2api.exe` を起動
4. ブラウザで `http://127.0.0.1:8080` を開く

#### デフォルトのデータ保存先

- データベース: `./data/sub2api.db`
- ログファイル: `./data/logs/sub2api.log`

これらのパスは既定で実行ファイルと同じディレクトリ配下に作成されるため、展開したフォルダをそのまま自己完結型の配備ディレクトリとして使えます。
...
Docker Compose でデプロイします。コンテナ化された複数サービス構成を使いたい場合は、この方法を引き続き利用できます。
...
> **補足:** データベースエンジンを明示しない場合、アプリケーションは既定で `sqlite` を使用し、デフォルトの DB ファイルは実行時ディレクトリ配下の `data/` に作成されます。
