...
> **当前默认部署方式：** Sub2API 现在默认使用 **内嵌 SQLite3**，单机启动时 **不再依赖 PostgreSQL / Redis**。因此可以直接在 **Windows** 上使用发布版二进制启动。
...
| 数据库 | SQLite3（内嵌，默认） |
| 缓存/队列 | 默认单机内置模式；Redis 在部分部署场景下可选 |
...
如果你只是想在本地或单台 Windows 机器上运行 Sub2API，可以直接使用 GitHub Releases 发布的二进制文件。

#### 步骤

1. 从 GitHub Releases 下载 Windows 发布包
2. 解压到任意目录，例如 `D:\sub2api`
3. 运行 `sub2api.exe`
4. 在浏览器打开 `http://127.0.0.1:8080`

#### 默认数据位置

- 数据库文件：`./data/sub2api.db`
- 日志文件：`./data/logs/sub2api.log`

这些路径默认相对于可执行文件所在目录创建，因此解压后的整个目录可以直接作为自包含部署目录使用。
...
使用 Docker Compose 部署。如果你仍然希望使用容器化的多服务模式，仍可继续使用这种方式。
...
> **说明：** 如果未显式指定数据库引擎，程序默认使用 `sqlite`，并将默认数据库文件放在运行目录下的 `data/` 子目录中。
